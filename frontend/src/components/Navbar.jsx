      ...
      <nav className="relative flex w-full flex-nowrap items-center justify-between bg-zinc-800 py-2 text-white lg:flex-wrap lg:justify-start lg:py-4">
        <div className="flex w-full flex-wrap items-center justify-between px-3">
          ...
          <div className="w-1/6 block lg:hidden">
            <button
              className="block border-0 bg-transparent px-2 hover:no-underline focus:outline-none"
              type="button"
          ...
          <div className="5/6 hidden lg:block">
            <div className="flex items-center">
              {links.map((items, i) => (
                <div
                  key={i}
                  className={`mx-3 rounded transition-all duration-300 hover:cursor-pointer ${
                    items.title === "Profile" || items.title === "Admin Profile"
                      ? "border border-blue-500 px-3 py-1 hover:bg-white hover:text-zinc-900"
                      : "hover:text-blue-300"
                  }`}
                >
      ...
      <div className={`5/6 ${Nav} lg:hidden bg-zinc-800 text-white px-12`}>
        <div className="flex flex-col items-center">
          {links.map((items, i) => (
            <div
              key={i}
              className={`my-3 mx-3 rounded transition-all duration-300 hover:cursor-pointer ${
                items.title === "Profile" || items.title === "Admin Profile"
                  ? "border border-blue-500 px-3 py-1 hover:bg-white hover:text-zinc-900"
                  : "hover:text-blue-300"
              }`}
